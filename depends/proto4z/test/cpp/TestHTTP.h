...
public:
    bool Test(WriteHTTP &wh)
    {
        ...
        HTTPHeadMap head;
        std::string body;
        if (checkHTTPBuffIntegrity(wh.getStream(), wh.getStreamLen(), 1024, false, isChunked, commandLine, head, body).first == IRT_SUCCESS)
        {
        ...
        if (checkHTTPBuffIntegrity(wh.getStream(), wh.getStreamLen() - 1, 1024, false, isChunked, commandLine, head, body).first != IRT_SHORTAGE)
        {
            cout << "Check checkHTTPBuffIntegrity IRT_SHORTAGE error" << endl;
            return false;
        }
        if (checkHTTPBuffIntegrity(wh.getStream(), wh.getStreamLen(), wh.getStreamLen() - 1, false, isChunked, commandLine, head, body).first != IRT_CORRUPTION)
        {
